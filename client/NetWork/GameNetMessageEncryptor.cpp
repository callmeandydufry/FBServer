...
//------------------------------------------------------------------------------------------------------------
NetStreamPointer::NetStreamPointer(uchar* pBuf, uint32 uHead, uint32 uTail, uint32 uBufMaxSize)
{
	m_pPointer = pBuf;
	m_uHead = uHead;
	m_uTail = uTail;
...
NetStreamPointer::~NetStreamPointer()
{}

uchar* NetStreamPointer::GetPointer()
{
	return m_pPointer + m_uHead;
}

uchar* NetStreamPointer::GetPointer(uint32 offset)
{
...
NetStreamPointer& NetStreamPointer::operator += (uint32 u)
{
	m_uHead += u;
...
NetStreamPointer& NetStreamPointer::operator ++ ()
{
	m_uHead++;
	if (m_uHead >= m_uBufMaxSize)
	{
...
void NetStreamPointer::CopyData(uchar* pDst, uint32 uSize)
{
...
uint32 NetStreamPointer::GetSize()
{
	uint32 uSize = 0;
	if (m_uTail >= m_uHead)
...
//------------------------------------------------------------------------------------------------------------
NetShuffleDataArray::ShuffleData NetShuffleDataArray::m_Array[MAX_SHUFFLE_DATA_ARRAY_SIZE] =
{
...
NetShuffleDataArray::ShuffleData* NetShuffleDataArray::GetShuffleData(uint32 uIndex)
{
...
NetEncryptFunction::func_ptr NetEncryptFunction::m_pDoFuncPtr[MAX_ENCRYPT_FUNC_SIZE] =
{
	DoFunc0,
	DoFunc1,
	DoFunc2,
...
NetEncryptFunction::func_ptr NetEncryptFunction::m_pDeFuncPtr[MAX_ENCRYPT_FUNC_SIZE] =
{
	DeFunc0,
	DeFunc1,
	DeFunc2,
...
NetEncryptFunction::func_ptr NetEncryptFunction::GetDoFuncPtr(uint32 uIndex)
{
...
NetEncryptFunction::func_ptr NetEncryptFunction::GetDeFuncPtr(uint32 uIndex)
{
	if (uIndex >= MAX_ENCRYPT_FUNC_SIZE)
		return NULL;
...
void NetEncryptFunction::DoFunc0(uchar* p, uint32 uMagic) { *p += 50; }
void NetEncryptFunction::DoFunc1(uchar* p, uint32 uMagic) { *p += 94; }
void NetEncryptFunction::DoFunc2(uchar* p, uint32 uMagic) { *p += 163; }
void NetEncryptFunction::DoFunc3(uchar* p, uint32 uMagic) { *p += 214; }
void NetEncryptFunction::DoFunc4(uchar* p, uint32 uMagic) { *p = ((*p) >> 2) | ((*p) << 6); }
void NetEncryptFunction::DoFunc5(uchar* p, uint32 uMagic) { *p = ((*p) >> 3) | ((*p) << 5); }
void NetEncryptFunction::DoFunc6(uchar* p, uint32 uMagic) { *p = ((*p) >> 5) | ((*p) << 3); }
void NetEncryptFunction::DoFunc7(uchar* p, uint32 uMagic) { *p = ((*p) >> 7) | ((*p) << 1); }
void NetEncryptFunction::DoFunc8(uchar* p, uint32 uMagic) { *p = (*p) ^ uMagic; }
void NetEncryptFunction::DoFunc9(uchar* p, uint32 uMagic) { *p = (*p) ^ (uMagic >> 8); }
void NetEncryptFunction::DoFunc10(uchar* p, uint32 uMagic) { *p = (*p) ^ (uMagic >> 16); }
void NetEncryptFunction::DoFunc11(uchar* p, uint32 uMagic) { *p = (*p) ^ (uMagic >> 24); }

void NetEncryptFunction::DeFunc0(uchar* p, uint32 uMagic) { *p -= 50; }
void NetEncryptFunction::DeFunc1(uchar* p, uint32 uMagic) { *p -= 94; }
void NetEncryptFunction::DeFunc2(uchar* p, uint32 uMagic) { *p -= 163; }
void NetEncryptFunction::DeFunc3(uchar* p, uint32 uMagic) { *p -= 214; }
void NetEncryptFunction::DeFunc4(uchar* p, uint32 uMagic) { *p = ((*p) >> 6) | ((*p) << 2); }
void NetEncryptFunction::DeFunc5(uchar* p, uint32 uMagic) { *p = ((*p) >> 5) | ((*p) << 3); }
void NetEncryptFunction::DeFunc6(uchar* p, uint32 uMagic) { *p = ((*p) >> 3) | ((*p) << 5); }
void NetEncryptFunction::DeFunc7(uchar* p, uint32 uMagic) { *p = ((*p) >> 1) | ((*p) << 7); }
void NetEncryptFunction::DeFunc8(uchar* p, uint32 uMagic) { *p = (*p) ^ uMagic; }
void NetEncryptFunction::DeFunc9(uchar* p, uint32 uMagic) { *p = (*p) ^ (uMagic >> 8); }
void NetEncryptFunction::DeFunc10(uchar* p, uint32 uMagic) { *p = (*p) ^ (uMagic >> 16); }
void NetEncryptFunction::DeFunc11(uchar* p, uint32 uMagic) { *p = (*p) ^ (uMagic >> 24); }

//------------------------------------------------------------------------------------------------------------
bool NetMessageEncryptor::Shuffle(MSG_ID MsgID, uchar* pBuf, uint32 uHead, uint32 uTail, uint32 uBufMaxSize)
{
	return _Shuffle(MsgID, pBuf, uHead, uTail, uBufMaxSize, eMode_Shuffle);
}

bool NetMessageEncryptor::DeShuffle(MSG_ID MsgID, uchar* pBuf, uint32 uHead, uint32 uTail, uint32 uBufMaxSize)
{
	return _Shuffle(MsgID, pBuf, uHead, uTail, uBufMaxSize, eMode_DeShuffle);
}

bool NetMessageEncryptor::_Shuffle(MSG_ID MsgID, uchar* pBuf, uint32 uHead, uint32 uTail, uint32 uBufMaxSize, eSHUFFLE_MODE eShuffleMode)
{
	uint32 uIndex = ((uint32)MsgID + m_uMagicNum) % MAX_SHUFFLE_DATA_ARRAY_SIZE;
	NetShuffleDataArray::ShuffleData *pShuffle = NetShuffleDataArray::GetShuffleData(uIndex);
	if (pShuffle == NULL)
		return false;

	NetStreamPointer Pt(pBuf, uHead, uTail, uBufMaxSize);
...
bool NetMessageEncryptor::Encrypt(MSG_ID MsgID, uchar* pBuf, uint32 uHead, uint32 uTail, uint32 uBufMaxSize)
{
	return _Encrypt(MsgID, pBuf, uHead, uTail, uBufMaxSize, eMode_Encrypt);
}

bool NetMessageEncryptor::DeEncrypt(MSG_ID MsgID, uchar* pBuf, uint32 uHead, uint32 uTail, uint32 uBufMaxSize)
{
	return _Encrypt(MsgID, pBuf, uHead, uTail, uBufMaxSize, eMode_DeEncrypt);
}

bool NetMessageEncryptor::_Encrypt(MSG_ID MsgID, uchar* pBuf, uint32 uHead, uint32 uTail, uint32 uBufMaxSize, eENCRYPT_MODE eEncryptMode)
{
	uint32 uIndex = (m_uMagicNum - (uint32)MsgID) % MAX_ENCRYPT_FUNC_SIZE;
	NetEncryptFunction::func_ptr pFunc = NULL;
	if (eEncryptMode == eMode_Encrypt)
		pFunc = NetEncryptFunction::GetDoFuncPtr(uIndex);
	else if (eEncryptMode == eMode_DeEncrypt)
		pFunc = NetEncryptFunction::GetDeFuncPtr(uIndex);
...
bool NetMessageEncryptor::Encode(MSG_ID MsgID, uchar* pBuf, uint32 uHead, uint32 uTail, uint32 uBufMaxSize)
{
...
bool NetMessageEncryptor::Decode(MSG_ID MsgID, uchar* pBuf, uint32 uHead, uint32 uTail, uint32 uBufMaxSize)
{
