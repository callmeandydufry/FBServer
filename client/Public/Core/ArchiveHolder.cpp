...
BOOL ArchiveNodeWrapper::Create(MK_VALUE key, uint32 Size)
{
	...
	if(m_hold == INVALID_KHAN_SM_HANDLE)
	{
		QLogSystem::QUICK_LOG(SLOW_LOG_MAPSERVER_SHAREMEM, "Create ShareMem Error SM_KET = %d", key);
		return FALSE;
	}
	...
	if(m_pHeader)
	{
		...
		m_Size = Size;
		QLogSystem::QUICK_LOG(SLOW_LOG_MAPSERVER_SHAREMEM, "Create ShareMem Ok SM_KET = %d", key);
		return TRUE;
	}
	else
	{
		QLogSystem::QUICK_LOG(SLOW_LOG_MAPSERVER_SHAREMEM, "Map ShareMem Error SM_KET = %d", key);
		return FALSE;
	}
...
BOOL ArchiveNodeWrapper::ViewMap(MK_VALUE key, uint32 Size)
{
	...
	if(m_hold == INVALID_KHAN_SM_HANDLE)
	{
		QLogSystem::QUICK_LOG(SLOW_LOG_MAPSERVER_SHAREMEM, "Attach ShareMem Error SM_KET = %d", key);
		return FALSE;
	}
	...
	if(m_pHeader)
	{
		...
		m_Size = Size;
		QLogSystem::QUICK_LOG(SLOW_LOG_MAPSERVER_SHAREMEM, "Attach ShareMem OK SM_KET = %d", key);
		return TRUE;
	}
	else
	{
...
ArchiveNodePool<PlayerMK>	*g_pPlayerMKPool = NULL;
