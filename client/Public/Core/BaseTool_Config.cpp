...
#ifdef __GLSERVER__
	QLogSystem::QUICK_LOG(SLOW_LOG_WORLD, "[Config] Begin Load %s ... ", filename);
#elif defined __MAPSERVER__
	QLogSystem::QUICK_LOG(SLOW_LOG_SERVER, "[Config] Begin Load %s ... ", filename);
#endif
...
#ifdef __GLSERVER__
	QLogSystem::QUICK_LOG(SLOW_LOG_WORLD, "[Config] End   Load %s ! ", m_szFileName);
#elif defined __MAPSERVER__
	QLogSystem::QUICK_LOG(SLOW_LOG_SERVER, "[Config] End   Load %s ! ", m_szFileName);
#endif
...
const char*	Configer::GetText(const char *section, const char *key, const char* strDefault)
{
	__GUARD__
...
BOOL Configer::HasSection(const char* section)
{
	__GUARD__
...
void Configer::_TrimString( char* buf )
{
	...
	size_t _Idx;
	for( _Idx=0; _Idx<strlen(buf); _Idx++ )
	{
		if( buf[_Idx] == CONFIGER_NOTE )
		{
			buf[_Idx] = '\0';
			break;
		}
	...
	for( _Idx=strlen(buf)-1; _Idx>=0; _Idx-- )
	{
		if( buf[_Idx]==' ' || buf[_Idx]=='\t' )
		{
			buf[_Idx] = 0;
		}
		else
